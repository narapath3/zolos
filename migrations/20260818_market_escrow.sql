...
-- Supabase signatures intentionally use auth.uid(); the self-hosted port in
-- server/api/rpc_functions.sql receives the verified user id explicitly.

DROP FUNCTION IF EXISTS public.create_market_listing(uuid, text, text, integer, integer);

CREATE OR REPLACE FUNCTION public.create_market_listing(
  p_character_id text, p_item_name text,
...
BEGIN
  ...
  VALUES
    ('item_' || substr(md5(random()::text || clock_timestamp()::text), 1, 24),
     v_inv.item_name, v_inv.item_type, p_quantity, p_price, auth.uid(), v_char.name,
...
REVOKE EXECUTE ON FUNCTION public.create_market_listing(text, text, integer, integer) FROM anon, public;
REVOKE EXECUTE ON FUNCTION public.cancel_market_listing(uuid) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.create_market_listing(text, text, integer, integer) TO authenticated;
