...
test('in-world stall signs use high-DPI Thai-safe fitted canvas text', () => {
  ...
  assert.match(scene, /const width = 1024/);
  assert.match(scene, /const height = 180/);
  assert.match(scene, /const panel = \{ x: 20, y: 18, width: 984, height: 144 \}/);
  assert.match(scene, /const titleBox = \{ x: 94, y: 43, width: 836, height: 94 \}/);
  assert.match(scene, /drawFittedCanvasText\(ctx, shopName \|\| 'ร้านค้า', width \/ 2, 104/);
  assert.match(scene, /function createShopSignSprite\(shopName, options = \{\}/);
  assert.match(scene, /createShopSignSprite\(stall\.shop_name, \{/);
  assert.match(scene, /createShopSignSprite\('ร้านค้า', \{/);
  assert.match(scene, /createShopSignSprite\('เปิดร้าน', \{/);
  assert.match(scene, /createShopSignSprite\('รับซื้อไอเทม', \{/);
  ...
  assert.match(scene, /texture\.minFilter = THREE\.LinearFilter/);
  assert.match(scene, /texture\.wrapS = THREE\.ClampToEdgeWrapping/);
  assert.match(scene, /texture\.wrapT = THREE\.ClampToEdgeWrapping/);
  assert.match(scene, /texture\.generateMipmaps = false/);
  assert.match(scene, /const worldWidth = touch \? 3\.55 : 4\.05/);
  assert.match(scene, /group\.userData\.shopLabel = sign/);
  assert.match(scene, /_updateShopLabelVisibility\(dt\)\s*\{/);
  assert.match(scene, /const targetOpacity = distance <= 11/);
  assert.match(scene, /depthTest: false,[\s\S]*depthWrite: false/);
  assert.match(scene, /const touch = \/Android\|iPad\|iPhone\|iPod\/i\.test\(navigator\.userAgent\)/);
  assert.match(scene, /window\.matchMedia\?\.\('\(pointer: coarse\)'\)\?\.matches/);
  assert.match(scene, /function drawFittedCanvasText\([^)]*maxLines = 1/);
  assert.match(scene, /ctx\.textAlign = 'center'/);
