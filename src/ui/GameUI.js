...
export class GameUI {
  ...
  showGuestExitWarning() {
    ...
    return new Promise(resolve => {
      ...
      bindTap(cancelButton, () => finish('cancel'));
      bindTap(leaveButton, () => finish('leave'));
      bindTap(bindButton, async () => {
        if (bindInFlight) return;
  ...
  openPetBoutique() {
    ...
    const ownedPetCount = this._allPetInstances().length;
    const getPetState = entry => {
      const item = this.inventory?.find(i => i.item_name === entry.name && i.item_type === 'pet');
      const instances = item ? this._ensurePetInstances(item) : [];
      const equipped = !!this.character?.equippedPetUid && instances.some(inst => inst.uid === this.character.equippedPetUid);
      return { item, ownedCount: instances.length, equipped };
    };
    ...
    const selectEntry=(entry,card,open=true)=>{
      ...
      const ownedLabel=state.equipped?'กำลังใช้งานอยู่':state.ownedCount?`มีในคอลเลกชัน ${state.ownedCount} ตัว`:'';
      const actionLabel=this.character.stats.gold<entry.price?'Zeny ไม่พอ':state.ownedCount?'รับเลี้ยงเพิ่ม':'รับเลี้ยงตัวนี้';
      const actionClass=state.equipped?' is-equipped':state.ownedCount?' is-owned':'';
      detail.innerHTML=`<button class="pet-detail__back" type="button">← กลับไปเลือกตัวอื่น</button><div class="pet-detail__art">${petModelMarkup(data.pet,560)||petPortraitMarkup(data.pet)}</div><div><span class="pet-detail__rarity">${data.rarity}</span><h3 class="pet-detail__name">${entry.name.replace(' Pet','')}</h3>${ownedLabel?`<span class="pet-detail__owned">${ownedLabel}</span>`:''}<p class="pet-detail__desc">${data.desc}</p>${combatHtml}<div class="pet-detail__price-row"><span>ราคา</span><strong class="pet-detail__price">${entry.price.toLocaleString()} Zeny</strong></div></div><button class="pet-detail__buy${actionClass}" ${this.character.stats.gold<entry.price?'disabled':''}>${actionLabel}</button>`;
    ...
    const petGrid=modal.querySelector('.pet-boutique__grid');
    const petFilters=[...modal.querySelectorAll('[data-pet-filter]')];
    const petSort=modal.querySelector('.pet-boutique__sort');
    let activePetFilter='all';
    const rarityOrder={common:0,rare:1,epic:2,legendary:3,mythic:4};
    const applyPetView=()=>{
      const cards=[...modal.querySelectorAll('.pet-card')];
      cards.sort((a,b)=>{
        if(petSort?.value==='price-asc') return Number(a.dataset.price)-Number(b.dataset.price);
        if(petSort?.value==='price-desc') return Number(b.dataset.price)-Number(a.dataset.price);
        if(petSort?.value==='rarity') return (rarityOrder[a.dataset.rarity]??99)-(rarityOrder[b.dataset.rarity]??99);
        return PET_SHOP.findIndex(x=>x.name===a.dataset.pet)-PET_SHOP.findIndex(x=>x.name===b.dataset.pet);
      }).forEach(card=>petGrid.appendChild(card));
      let visible=0;
      cards.forEach(card=>{
        const owned=card.dataset.owned==='1';
        const rarity=card.dataset.rarity;
        const matches=activePetFilter==='all'||activePetFilter===rarity||(activePetFilter==='owned'&&owned)||(activePetFilter==='available'&&!owned);
        card.style.display=matches?'':'none';
        if(matches) visible++;
      });
      let empty=petGrid.querySelector('.pet-card__empty');
      if(!visible){
        if(!empty){empty=document.createElement('div');empty.className='pet-card__empty';petGrid.appendChild(empty);}
        empty.textContent=activePetFilter==='owned'?'ยังไม่มีสัตว์เลี้ยงในคอลเลกชัน': 'ไม่พบสัตว์เลี้ยงในหมวดนี้';
        empty.style.display='block';
      }else if(empty) empty.style.display='none';
    };
    petFilters.forEach(filter=>filter.onclick=()=>{
      activePetFilter=filter.dataset.petFilter;
      petFilters.forEach(x=>x.classList.toggle('is-active',x===filter));
      applyPetView();
    });
    petSort?.addEventListener('change',applyPetView);
