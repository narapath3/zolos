...
export class GameUI {
  ...
  openPetBoutique() {
    ...
    if (!modal) {
      ...
      style.textContent += `.pet-boutique__body{min-height:0;display:grid;grid-template-columns:minmax(0,1fr) 290px;overflow:hidden}.pet-boutique__detail{margin:14px 14px 14px 0;padding:14px;border:1px solid #31466b;border-radius:14px;background:#0a1120;overflow:auto}.pet-detail__art{height:220px;border-radius:11px;overflow:hidden;background:#050b16}.pet-detail__art .pet-atlas-portrait{height:100%}.pet-detail__rarity{display:inline-block;margin-top:12px;padding:4px 8px;border-radius:6px;background:#263a5c;color:#ffe19a;font-size:10px;font-weight:900;text-transform:uppercase}.pet-detail__name{margin:7px 0 4px;color:#fff;font-size:21px}.pet-detail__desc{margin:0;color:#b8c7dc;font-size:12px;line-height:1.7}.pet-detail__price-row{display:flex;justify-content:space-between;align-items:center;margin-top:14px;padding:10px;border-radius:9px;background:#111c31}.pet-detail__price{color:#ffd464;font-size:17px;font-weight:900}.pet-detail__buy{width:100%;min-height:48px;margin-top:10px;border:1px solid #76a8ec;border-radius:10px;background:#356fb8;color:#fff;font-size:14px;font-weight:900;cursor:pointer;touch-action:manipulation}.pet-detail__buy:disabled{opacity:.45;filter:grayscale(1)}.pet-card.is-selected{border-color:#8ab8ff;box-shadow:0 0 0 2px rgba(94,155,239,.35)}.pet-detail__combat{margin-top:11px;padding:10px 11px;border:1px solid #3a2f52;border-radius:11px;background:linear-gradient(150deg,#1a1330,#12182b)}.pet-detail__combat-title{display:flex;justify-content:space-between;align-items:center;color:#ffdf9a;font-size:12px;font-weight:900;letter-spacing:.02em}.pet-detail__elem{font-size:11px;font-weight:900}.pet-detail__move{margin:6px 0 8px;color:#e7d6ff;font-size:11px;line-height:1.55}.pet-detail__stats{display:flex;gap:7px;flex-wrap:wrap}.pet-detail__stats span{flex:1;min-width:64px;text-align:center;padding:6px 4px;border-radius:8px;background:#0c1424;color:#9fb2cd;font-size:10px}.pet-detail__stats strong{display:block;color:#fff;font-size:15px;margin-top:2px}.pet-detail__scale{margin-top:8px;color:#8fe3b0;font-size:10px;text-align:center}@media(max-width:700px){.pet-detail__combat{grid-column:1/-1}.pet-boutique__body{display:flex;flex-direction:column;overflow:auto}.pet-boutique__detail{order:0;margin:7px;display:grid;grid-template-columns:112px 1fr;gap:9px;padding:8px;overflow:visible}.pet-detail__art{height:112px;grid-row:1/5}.pet-detail__rarity{margin-top:0;width:max-content}.pet-detail__name{font-size:16px;margin:2px 0}.pet-detail__desc{font-size:10px;line-height:1.4;max-height:42px;overflow:hidden}.pet-detail__price-row{margin-top:3px;padding:5px 7px}.pet-detail__price{font-size:13px}.pet-detail__buy{grid-column:1/-1;min-height:44px;margin-top:0}.pet-boutique__grid{order:1;overflow:visible}}`;
    ...
    modal.innerHTML = `<section class="pet-boutique" role="dialog" aria-modal="true" aria-label="Pet Sanctuary"><header class="pet-boutique__hero"><h2>✦ Pet Sanctuary</h2><p>แตะสัตว์เลี้ยงเพื่อดูรายละเอียด ราคา และรับเลี้ยง</p><div class="pet-boutique__wallet">Zeny ${gold.toLocaleString()}</div><button class="pet-boutique__close" aria-label="ปิด">×</button></header><div class="pet-boutique__body"><div class="pet-boutique__grid">${PET_SHOP.map(entry=>{const data=ITEMS[entry.name];return `<article class="pet-card" tabindex="0" data-pet="${entry.name}"><span class="pet-card__rarity">${data.rarity}</span><div class="pet-card__art">${petPortraitMarkup(data.pet)}</div><h3>${entry.name.replace(' Pet','')}</h3><div class="pet-card__foot"><span class="pet-card__price">${entry.price.toLocaleString()} z</span><button class="pet-card__buy" ${gold<entry.price?'disabled':''}>เลือก</button></div></article>`}).join('')}</div><aside class="pet-boutique__detail" aria-live="polite"></aside></div></section>`;
    ...
    const selectEntry=(entry,card)=>{
      ...
      detail.innerHTML=`<div class="pet-detail__art">${petPortraitMarkup(data.pet)}</div><div><span class="pet-detail__rarity">${data.rarity}</span><h3 class="pet-detail__name">${entry.name.replace(' Pet','')}</h3><p class="pet-detail__desc">${data.desc}</p>${combatHtml}<div class="pet-detail__price-row"><span>ราคา</span><strong class="pet-detail__price">${entry.price.toLocaleString()} Zeny</strong></div></div><button class="pet-detail__buy" ${this.character.stats.gold<entry.price?'disabled':''}>${this.character.stats.gold<entry.price?'Zeny ไม่พอ':'รับเลี้ยงตัวนี้'}</button>`;
    ...
    if(firstCard)selectEntry(PET_SHOP[0],firstCard);
