...
export class GameUI {
  ...
  openPetBoutique() {
    ...
    const selectEntry=(entry,card)=>{
      ...
      detail.innerHTML=`<div class="pet-detail__art">${petPortraitMarkup(data.pet)}</div><div><span class="pet-detail__rarity">${data.rarity}</span><h3 class="pet-detail__name">${entry.name.replace(' Pet','')}</h3><p class="pet-detail__desc">${data.desc}</p><div class="pet-detail__price-row"><span>ราคา</span><strong class="pet-detail__price">${entry.price.toLocaleString()} Zeny</strong></div></div><button class="pet-detail__buy" ${this.character.stats.gold<entry.price?'disabled':''}>${this.character.stats.gold<entry.price?'Zeny ไม่พอ':'รับเลี้ยงตัวนี้'}</button>`;
