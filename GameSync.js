...
export async function fetchMarketListings() {
    // 1. Always start with local listings (for items listed while offline or if Supabase failed)
    let allListings = initLocalMarketplace();
    ...
    try {
        ...
        if (error) {
            console.warn('[Zolos] Supabase marketplace query failed, showing local only:', error.message);
            return allListings;
        }

        if (data && data.length > 0) {
            // Merge online into local, avoiding duplicates by ID
            const localIds = new Set(allListings.map(l => l.id));
            const uniqueOnline = data.filter(l => !localIds.has(l.id));
            allListings = [...allListings, ...uniqueOnline];
        ...
        console.warn('[Zolos] Catch error on fetching marketplace, showing local only:', err.message);
        return allListings;
    }
}
