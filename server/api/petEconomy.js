...
export const PET_CATALOG = Object.freeze({
    'Poring Pet': { price: 2000, pet: 'poring', rarity: 'common' },
    'Chick Pet': { price: 3000, pet: 'chick', rarity: 'common' },
    'Kitten Pet': { price: 8000, pet: 'kitten', rarity: 'rare' },
    'Puppy Pet': { price: 10000, pet: 'puppy', rarity: 'rare' },
    'Sunfox Pet': { price: 18000, pet: 'sunfox', rarity: 'rare' },
    'Moss Turtle Pet': { price: 22000, pet: 'moss_turtle', rarity: 'rare' },
    'Owl Pet': { price: 25000, pet: 'owl', rarity: 'epic' },
    'Cloudling Pet': { price: 42000, pet: 'cloudling', rarity: 'epic' },
    'Moon Hare Pet': { price: 55000, pet: 'moon_hare', rarity: 'epic' },
    'Baby Dragon Pet': { price: 60000, pet: 'baby_dragon', rarity: 'legendary' },
    'Bloom Fairy Pet': { price: 95000, pet: 'bloom_fairy', rarity: 'legendary' },
    'Ember Phoenix Pet': { price: 180000, pet: 'ember_phoenix', rarity: 'mythic' },
});

export async function ensurePetEconomy() {
    await query(`CREATE TABLE IF NOT EXISTS public.pet_purchase_requests (
    ...
    await query(PURCHASE_PET_SQL);
    await query(`REVOKE ALL ON TABLE public.pet_purchase_requests FROM PUBLIC`);
    await query(`REVOKE ALL ON FUNCTION public.purchase_pet(text, text, integer, text, text) FROM PUBLIC`);
    await query(`GRANT EXECUTE ON FUNCTION public.purchase_pet(text, text, integer, text, text) TO CURRENT_USER`);
...
DECLARE
  ...
  v_uid text;
  v_result jsonb;
  v_receipt_character text;
  v_receipt_item text;
BEGIN
  IF p_character_id IS NULL OR p_character_id = ''
     OR p_item_name IS NULL OR p_item_name = ''
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_character_id || ':' || p_idempotency_key, 0));
  SELECT character_id, item_name, result
    INTO v_receipt_character, v_receipt_item, v_result
  FROM public.pet_purchase_requests
  WHERE idempotency_key = p_idempotency_key;
  IF FOUND THEN
    IF v_receipt_character IS DISTINCT FROM p_character_id
       OR v_receipt_item IS DISTINCT FROM p_item_name THEN
  ...
  v_uid := 'pet_' || pg_catalog.substr(
    pg_catalog.md5(p_character_id || ':' || p_idempotency_key), 1, 24);
  ...
  v_stats := coalesce(v_stats, '{}'::jsonb)
    || pg_catalog.jsonb_build_object('instances', v_instances);
  ...
  SET gold = gold - p_price, updated_at = pg_catalog.now()
  WHERE id = p_character_id
